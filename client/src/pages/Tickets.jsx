...
const CustomDropdown = ({ id, name, value, onChange, options, label, className = "" }) => {
  ...
  return (
    <div className="relative" ref={dropdownRef}>
      ...
      {isOpen && (
        <div className="absolute z-[100] mt-1 w-full bg-white border border-secondary-200 rounded-md shadow-xl max-h-72 overflow-y-auto min-w-[200px]">
          {options.map((option) => (
            <div
              key={option.value}
              className={`px-4 py-3 cursor-pointer text-base hover:bg-primary-50 ${value === option.value ? 'bg-primary-100 text-primary-800' : 'text-secondary-800'}`}
              onClick={() => {
...
const Tickets = () => {
  ...
  const handleAddTicket = async (e) => {
    ...
    try {
      ...
      // Salva la descrizione come testo puro
            ...
            <div className="space-y-3">
              {filteredTickets.map((ticket, index) => (
                <div 
                  ...
                  className="bg-white rounded-xl shadow-sm border border-secondary-200 hover:shadow-md hover:border-primary-200 transition-all overflow-hidden cursor-pointer group p-6 min-h-[120px] text-base"
                        ...
                        <div className="flex items-center justify-between text-xs">
                          ...
                          >
                            <CustomDropdown
                              ...
                              className="py-2 px-4 text-xs font-medium rounded-full min-w-[110px] h-7 flex items-center justify-center border-0 shadow-none bg-blue-100 text-blue-700"
                              options={[
                                { value: "CLOSED", label: "Chiuso" },
                                { value: "OPEN", label: "Aperto" },
                                { value: "PLANNED", label: "Painificato" },
